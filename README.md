...
*  **Визуальный анализ VLM:** Скриншот страницы отправляется в `Gemini 2.0 Flash` (via OpenRouter), где извлекаются 4 контекстных признака: наличие кнопки CTA, яркость её цвета (HEX), тональность текста и визуальная перегруженность.
**Thompson Bandit (PyTorch):** Обучение байесовских матриц $A$ и $b$ на лету с минимальными затратами CPU-памяти.
**Легковесный Frontend-SDK:** Vanilla JS скрипт, делающий скриншот DOM-дерева (`html2canvas`) и мгновенно переключающий выигрышный CSS-контекст. Устранена утечка памяти сессий (TTLCache).
...
**Supabase Persistence:** Веса бандита безопасно дампируются из PyTorch тензоров в Supabase в формате `base64`.
...
Инструмент сборки: [astral-sh/uv](https://github.com/astral-sh/uv).
